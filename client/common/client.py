...
class Client:
    ...
    def _listen_for_responses(self):
        ...
        try:
            while True:
                try:
                    ...
                    if self.received_data_ends >= self.expected_data_ends:
                        ...
                        # This prevents duplicate reconnection attempts from different threads
                        break
                except Exception as e:
            ...
            # Only close if we successfully received all DATA_END signals
            # Otherwise, let the main loop handle cleanup and reconnection
            if self.received_data_ends >= self.expected_data_ends:
                self.close()
    ...
    def start_client_loop(self):
        ...
        while True:
            try:
                ...
                # Check if we received all expected DATA_END signals
                if self.received_data_ends >= self.expected_data_ends:
                    print("[INFO] All query results received successfully")
                    # Success - break out of retry loop
                    break
                else:
                    # Listener thread exited prematurely (connection lost)
