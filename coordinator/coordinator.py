...
class Coordinator(StreamProcessingWorker):
    def __init__(self, queue_in, queue_out, rabbitmq_host, num_replicas):
        ...
        # Track END messages by (msg_type, data_type) tuple to distinguish different END types
        self.end_messages_received = defaultdict(int)  
        self.end_messages_forwarded = set()  # Track which (msg_type, data_type) have already had END forwarded
    ...
    def _handle_data_message(self, msg_type, data_type, request_id, timestamp, payload):
        ...
        if self.data_messages_count[data_type] <= 10 or self.data_messages_count[data_type] % 1000 == 0:
            logging.info(f"Forwarded data message #{self.data_messages_count[data_type]} (data_type: {data_type}, payload_size: {len(payload)} bytes)")
        else:
    ...
    def _handle_end_message(self, msg_type, data_type, request_id, timestamp, payload):
        ...
        logging.debug(f"Received END message {self.end_messages_received[end_key]}/{self.num_replicas} for msg_type: {msg_type}, data_type: {data_type}")
        
        # Only forward END message once after receiving from all replicas
        if (self.end_messages_received[end_key] >= self.num_replicas and 
            ...
            logging.info(f"Forwarded END message for msg_type: {msg_type}, data_type: {data_type} after receiving from all {self.num_replicas} replicas. Total data messages forwarded: {total_data_msgs}")
